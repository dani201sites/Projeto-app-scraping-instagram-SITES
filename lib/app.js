...
async function generateAiPromptForProfile({ profile, city }) {
  ...
  try {
    ...
    const content = normalizeAiContent(result.choices?.[0]?.message?.content);
...
function buildOpenRouterPayload({ model, profile, city }) {
  return {
    model,
    temperature: 0.25,
    max_tokens: 1200,
    messages: [
      ...
      },
      {
        ...
        content: JSON.stringify({
          task: "Analise os dados da barbearia e extraia informacoes para montar um prompt de site.",
          expectedJson: {
            businessName: "nome comercial mais provavel",
            city: "cidade",
            palette: ["cor principal", "cor secundaria", "cor de apoio"],
