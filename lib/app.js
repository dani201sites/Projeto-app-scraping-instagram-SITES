...
async function handleCreateTrelloCards(request, response) {
  ...
  const maxScreenshots = clamp(Number(process.env.MAX_SCREENSHOTS_PER_RUN || 3), 0, 30);

  for (const profile of leads) {
    const allowScreenshot = screenshotAttempts < maxScreenshots;
    const aiResult = aiPromptsGenerated < maxAiPrompts
      ? await generateAiPromptForProfile({ profile, city, allowScreenshot })
      : { enabled: isAiPromptGenerationEnabled(), prompt: null, error: "Limite de prompts da IA atingido nesta execucao." };

    screenshotAttempts += aiResult.screenshotAttempted ? 1 : 0;
    screenshotsCaptured += aiResult.screenshotCaptured ? 1 : 0;
...
async function generateAiPromptForProfile({ profile, city, allowScreenshot = false }) {
  ...
  const visualProfile = await prepareProfileVisualContext({ profile, allowScreenshot });
  const payload = buildOpenRouterPayload({ model, profile: visualProfile, city });
  ...
  try {
    ...
    if (!result.ok && shouldRetryOpenRouterWithoutImages(result.error)) {
      const textOnlyPayload = buildOpenRouterPayload({ model, profile: visualProfile, city, forceTextOnly: true });
      ...
      if (textOnlyResult.ok) {
        ...
        return {
          ...
          analysis: fallbackAnalysis,
          prompt,
          screenshotAttempted: Boolean(visualProfile.screenshotAttempted),
          screenshotCaptured: Boolean(visualProfile.screenshotDataUrl)
        };
      }
    ...
    const analysis = parseAiAnalysis(content);
    const prompt = buildWebsitePrompt({ profile: visualProfile, city, analysis });
...
async function prepareProfileVisualContext({ profile, allowScreenshot }) {
  if (!allowScreenshot || getScreenshotProvider() !== "browserless" || !isBrowserlessConfigured()) {
    return profile;
  }

  const screenshot = await captureBrowserlessScreenshot(profile);

  if (screenshot.dataUrl) {
    return {
      ...profile,
      screenshotDataUrl: screenshot.dataUrl,
      screenshotSource: "browserless",
...
async function captureBrowserlessScreenshot(profile) {
  const token = process.env.BROWSERLESS_API_KEY;

  if (!token) {
    return {
      dataUrl: null,
      error: "BROWSERLESS_API_KEY nao configurada."
    };
  }

  const baseUrl = String(process.env.BROWSERLESS_BASE_URL || "https://production-sfo.browserless.io").replace(/\/$/, "");
  const url = new URL(`${baseUrl}/screenshot`);
  url.searchParams.set("token", token);
  const instagramUrl = profile.instagramUrl || `https://www.instagram.com/${profile.handle}/`;
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        url: instagramUrl,
        bestAttempt: true,
        waitForTimeout: Number(process.env.BROWSERLESS_WAIT_MS || 3500),
        gotoOptions: {
          waitUntil: "domcontentloaded",
          timeout: Number(process.env.BROWSERLESS_NAVIGATION_TIMEOUT_MS || 15000)
        },
        viewport: {
          width: Number(process.env.BROWSERLESS_VIEWPORT_WIDTH || 1280),
          height: Number(process.env.BROWSERLESS_VIEWPORT_HEIGHT || 1000),
          deviceScaleFactor: 1
        },
        options: {
          type: "png",
    ...
    if (!response.ok || !contentType.includes("image/")) {
      ...
      return {
        dataUrl: null,
        error: text || `Browserless retornou HTTP ${response.status}`
      };
    }
...
function buildOpenRouterPayload({ model, profile, city, forceTextOnly = false }) {
  const imageInputs = forceTextOnly ? [] : getProfileVisualInputs(profile);
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              ...
              imageInstructions: imageInputs.length > 0
                ? "Use as imagens anexadas para inferir cores predominantes, estilo visual, materiais, tom de marca e direcao estetica. Se a imagem parecer tela de login, erro ou CAPTCHA, informe isso em warnings."
              ...
              profile: {
                ...
                screenshotSource: profile.screenshotSource || null,
                screenshotError: profile.screenshotError || null
              }
            })
          },
          ...imageInputs.map((url) => ({
...
function buildWebsitePrompt({ profile, city, analysis }) {
  ...
  const warnings = [
    ...
    profile.screenshotError ? `Screenshot nao utilizado: ${cleanText(String(profile.screenshotError))}` : null
  ].filter(Boolean);
  ...
  const visualNotes = [
    ...
    profile.screenshotDataUrl ? "Referencia visual analisada: screenshot temporario da primeira dobra do Instagram via Browserless." : null,
...
function getScreenshotProvider() {
  return String(process.env.SCREENSHOT_PROVIDER || "none").trim().toLowerCase();
}

function isBrowserlessConfigured() {
  return Boolean(process.env.BROWSERLESS_API_KEY);
}

function getProfileVisualInputs(profile) {
  const urls = [
    profile.profilePictureUrl,
    ...(Array.isArray(profile.visualImageUrls) ? profile.visualImageUrls : [])
  ];
  const inputs = [
    profile.screenshotDataUrl,
    ...normalizeImageUrls(urls)
  ].filter(Boolean);

  return inputs.slice(0, getMaxAiImagesPerPrompt());
}
